...
class BluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    // Enable or disable microphone on the glasses
    func enableMicrophone(_ enable: Bool, result: @escaping FlutterResult) {
        let command: UInt8 = enable ? 0x01 : 0x00
        let data = Data([0x0E, command])
        writeData(writeData: data)
        result("Microphone \(enable ? "enabled" : "disabled")")
    }

    // Get the audio stream from the glasses
    func onAudioDataReceived(_ data: Data) {
        // Convert the data to a hex string
        let hexString = data.map { String(format: "%02hhx", $0) }.joined()
        print("Received audio data: \(hexString)")
        
        // Send the audio data to the React Native side
        blueInfoSink?(["type": "audio", "data": hexString])
    }
